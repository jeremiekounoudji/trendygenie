...
class OrderController extends GetxController {
  ...
  Future<bool> fetchRecentOrders(String providerId) async {
    try {
      ...
      final response = await supabase
          ...
          .select('''
            *,
            ...
            business:companies!business_id (
              *,
              category:categories (
                *
              )
          ...
          ''')
          .eq('business.owner_id', providerId)
          .order('order_date', ascending: false)
          .limit(10)
          .execute();

      // Check both status and if there's data
      ...
      // Log for debugging
      log('Found ${data.length} orders');
      log('Found ${data.toString()} orders');
  ...
  Future<bool> fetchBusinessOrders(String businessId, int page, int limit, {String? status}) async {
    try {
      ...
      // Apply filters
      final filteredQuery = status != null 
          ? query.eq('business_id', businessId).eq('status', status)
  ...
  Future<bool> fetchDashboardMetrics(String providerId) async {
    try {
      ...
      if (businessesResponse.status != 200) {
        throw 'Error fetching businesses: Status ${businessesResponse.status}';
      }
      
      final businesses = businessesResponse.data as List;
      ...
      if (metricsResponse.status != 200) {
        throw 'Error fetching metrics: Status ${metricsResponse.status}';
      }
